...
class alrightHttpsRequest : public std::enable_shared_from_this<alrightHttpsRequest> {
 public:
  explicit alrightHttpsRequest(clientEndpointData aData, boost::asio::io_context& mIOcontext)
    ...
    mSocket.set_verify_callback([this](bool preverified, boost::asio::ssl::verify_context& aCtx) -> bool {
 ...
 private:
  void connectToSocket(const boost::system::error_code& errCode, boost::asio::ip::tcp::resolver::results_type mRes) {
    if (!errCode) {
      boost::asio::async_connect(
        mSocket.lowest_layer(), mRes,
        [sharedThis = shared_from_this()](const boost::system::error_code& errCode, const tcp::endpoint& /*endpoint*/) {
  ...
  void writeToSocket(const boost::system::error_code& errCode) {
    if (!errCode) {
      ...
      std::string requestString((std::istreambuf_iterator<char>(file)),
                                std::istreambuf_iterator<char>());  // remember to add a extra carriage return
