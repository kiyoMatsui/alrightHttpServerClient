...
class alrightHttpRequest : public std::enable_shared_from_this<alrightHttpRequest> {
 public:
  ...
  void resolveQuery() {
    // endpoints
    boost::asio::ip::tcp::resolver::query queryToResolve(mData.address, std::to_string(mData.portNumber),
 ...
 private:
  void connectToSocket(const boost::system::error_code& errCode, boost::asio::ip::tcp::resolver::iterator iterator) {
    if (!errCode) {
      mSocket.async_connect(*iterator, [sharedThis = shared_from_this()](const boost::system::error_code& errCode) {
  ...
  void writeToSocket(const boost::system::error_code& errCode) {
    if (!errCode) {
      ...
      // std::string requestString((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>()); //remember
      // to add a extra carriage return
      std::string requestString = mData.requestMethod + " " + mData.requestURI + " " + mData.requestHttpVersion +
                                  "\r\nHost: " + mData.address + ":" + std::to_string(mData.portNumber) +
