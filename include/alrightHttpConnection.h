...
class httpConnection : public std::enable_shared_from_this<httpConnection> {
 ...
 private:
  ...
  void requestReceived(const boost::system::error_code& errCode, std::size_t /*transferred_size_t*/) {
    ...
    lResponseStatusLine = "HTTP/1.1 " + std::to_string(lStatusCode) + " status line end" + "\r\n";
    lResponseHeaders = std::string("Server: ") + "alrightServer" + "\r\n" + std::string("content-length") + ": " +
  ...
  void shutdown(const boost::system::error_code& errCode, std::size_t transferred_size_t) {
    if (errCode) {
      systemError(errCode);
    }
